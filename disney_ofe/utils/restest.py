...
class Restest():
    ...
    def __send(self, serverUrl, method, endpointPath, params, data, json, headers, cookies, files, auth, timeout,
        ...
        if logRequest:
            self.logging.debug('method: {method}\nurl: {url}\nparams: {params}\njson: {json}\nheaders: {headers}\n'
                               'data: {data}\ntimeout: {timeout}\nfiles:{files}'.
                               format(method = method, url = url, params = params,
        ...
        try:
            ...
            if files:
                if isinstance(files, list):
                    ...
                    self.logging.error("Input files is not of type list or str")
            ...
            if method.lower() == 'get':
                ...
                response = requests.patch(url, params = params, data = data, json = json, headers = headers, cookies = cookies,
                                        files = filesObj, auth = auth, timeout = timeout, allow_redirects = allowRedirects,
                                        proxies = proxies, verify = verify, stream = stream, cert = cert)

            else:
                self.logging.error("method should be one of the value in ['get','post','put','delete','patch']")
                raise Exception("method should be one of the value in ['get','post','put','delete','patch']")
            ...
            if response is not None:
                try:
                    self.logging.error("Error from exception part Response: \"{0}\" Status Code: \"{1}\" Reason: \"{2}\" Elapsed time: \"{3}\"".
        ...
        if logResponse:
            try:
                self.logging.debug("Response: \"{0}\" Status Code: \"{1}\" Reason: \"{2}\" Elapsed time: \"{3}\"".
                             format(response.text, response.status_code, response.reason, response.elapsed.total_seconds()))
            except:
                self.logging.debug("Response: \"{0}\" Status Code: \"{1}\" Reason: \"{2}\" Elapsed time: \"{3}\"".
                             format(response.text.encode('utf-8'), response.status_code, response.reason, response.elapsed.total_seconds()))
    ...
    def __assertJsonEqual(self, json1, json2):
        ...
        try:
            assert (orderedJson(json1) == orderedJson(json2))
        except:
    ...
    def __validateJson(self, expectedJson, responseJson, logValidationErrors = True):
        ...
        try:
            if logValidationErrors and not isinstance(responseJson, type(expectedJson)):
                self.logging.error("Expected input: {0} type: {1} is not matching with actual input: {2} type: {3}".
                ...
                for value in expectedJson:
                    if isinstance(value, dict):
                        ...
                        if logValidationErrors and not dictFound:
                            self.logging.error("Expected dict: {0} not in actual JSON: {1}".
                                               format(value, responseDictList))
                        assert dictFound
                    else:
                        if logValidationErrors and value not in responseJson:
                            self.logging.error("Expected value: {0} is not in actual JSON: {1}".
                                               format(value, responseJson))
                ...
                for key, value in expectedJson.items():
                    if isinstance(value, (list, dict)):
                        ...
                        if logValidationErrors and responseJson[key] != value:
                            self.logging.error("Expected val: {0} and actual val: {1} for key: {2} not matching".format(value, responseJson[key], key))
        ...
        for itemDict in expectedResponse:
            ...
            if "comparator" in itemDict and "elapsedTime" in itemDict:
                ...
                if elapsedTimeError and errorLevel.lower() == 'error':
                    ...
                    self.logging.warning(elapsedTimeError)
            ...
            if itemDict.get("validator") == "schema":
                ...
                try:
                    ...
                    if errorLevel.lower() == 'error':
                        ...
                        self.hasWarnings = True
                        self.logging.warning("Schema is not matched. Reason: {0}, for Schema: {1}".
